...
```
描述：The GetComputerName function retrieves the NetBIOS name of the local computer.
...
gethostname函数：
```
描述：This function returns the standard host name for the local machine.
...
gethostname(name,20);
```
gethostbyname函数：从主机数据库中得到对应的主机的相应信息，参数name是主机名字，一般是由gethostname获得
```
描述：This function retrieves host information corresponding to a host name from a host database.
```
...
```
gethostbyaddr函数：从网络地址得到对应的主机.参数addr为地址，len为地址的长度，type为地址类型，eg：AF_INET
```
描述：This function retrieves the host information corresponding to a network address.
```
...
```
描述：MSDN: This function retrieves adapter information for the local computer.
```
...
```
描述：MSDN: This function retrieves network parameters for the local computer.
```
...
四、获取计算机安装的协议,可以通过函数WSAEnumProtocols获取安装在本地主机上可用的网络协议集
```
描述：MSDN：This function retrieves information about available transport protocols.
```
...
```
描述：MSDN: The getprotobyname function retrieves the protocol information corresponding to a protocol name.
```
...
五、获取计算机提供的服务,可以调用函数getservbyname来获取对应给定服务名和服务协议的相关服务信息
```
描述：MSDN: The getservbyname function retrieves service information corresponding to a service name and protocol.
```
...
六、获取计算机的所有网络资源,函数WNetOpenEnum开始一个网络连接枚举值,函数WNetEnumResource获取网络资源
```
DWORD WNetOpenEnum(
  DWORD dwScope,
  DWORD dwType,
...
```
3.删除IP地址，DeleteIpAddress:
>DWORD DeleteIPAddress(ULONG NTEContext);

八、获取计算机TCP/IP的所有信息,通过函数GetTcpStatics获取本地的TCP协议统计信息：
>DWORD GetTcpStatistics( PMIB_TCPSTATS pStats );

参数pStats是指向MIB_TCPSTATS的指针，定义如下：
```
